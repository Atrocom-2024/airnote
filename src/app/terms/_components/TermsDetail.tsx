export default function TermsDetail() {
  return (
    <section className="w-full max-h-[60vh] overflow-y-auto border-[1.5px] border-default rounded-md p-2 space-y-1">
      <h2>제1장 총칙</h2>
      <h3>제1조 (목적)</h3>
      ...
      <ul>
        ...
        <li>“계정정보”란 회원의 회원번호와 외부계정정보 기기정보 별명 프로필 사진 친구목록 등 회원이 회사에 제공한 정보와 이용정보 이용요금 결제 정보 등을 통칭합니다.</li>
        <li>“콘텐츠”란 모바일 기기로 이용할 수 있도록 회사가 서비스 제공과 관련하여 디지털 방식으로 제작한 유료 또는 무료의 내용물 일체(네트워크 서비스 애플리케이션 등)를 의미합니다.</li>
        <li>“오픈마켓”이란 모바일 기기에서 콘텐츠를 설치하고 결제할 수 있도록 구축된 전자상거래 환경을 의미합니다.</li>
        <li>“애플리케이션”이란 회사가 제공하는 서비스를 이용하기 위하여 모바일 기기를 통해 다운로드 받거나 설치하여 사용하는 프로그램 일체를 의미합니다.</li>
        <li>“서비스”라 함은 회사가 제공하는 서비스의 하나로서 회원이 모바일 기기에서 실행하는 온라인 매체나 모바일 PC 등을 이용한 서비스 및 이에 부수하는 서비스를 의미합니다.</li>
      </ul>
      ...
      <ul>
        <li>상호 및 운영주체</li>
        <li>전화번호 전자우편주소</li>
        <li>서비스 이용약관 (개인정보 처리방침)</li>
      </ul>

      <h3>제4조 (약관의 효력 및 변경)</h3>
      <p>① 회사는 이 약관의 내용을 회원이 알 수 있도록 서비스 내 또는 그 연결화면에 게시합니다. 이 경우 이 약관의 내용 중 서비스 중단 청약철회 환급 계약 해제․해지 회사의 면책사항 등과 같은 중요한 내용은 굵은 글씨 색채 부호 등으로 명확하게 표시하거나 별도의 연결화면 등을 통하여 회원이 알아보기 쉽게 처리합니다.</p>
      <p>② 회사가 약관을 개정할 경우에는 적용일자 및 개정내용 개정사유 등을 명시하여 최소한 그 적용일 7일 이전부터 서비스 내 또는 그 연결화면에 게시하여 회원에게 공지합니다. 다만 변경된 내용이 회원에게 불리하거나 중대한 사항의 변경인 경우에는 그 적용일 30일 이전까지 본문과 같은 방법으로 공지하고 제28조 제1항의 방법으로 회원에게 통지합니다. 이 경우 개정 전 내용과 개정 후 내용을 명확하게 비교하여 회원이 알기 쉽도록 표시합니다.</p>
      ...
      <p>② 회사는 가입신청자의 신청에 대하여 승낙함을 원칙으로 합니다. 다만 회사는 다음 각 호의 어느 하나에 해당하는 이용 신청에 대해서는 승낙을 거절할 수 있습니다.</p>
      <ul>
        <li>이용신청서 내용을 허위로 기재하거나 이용신청 요건을 충족하지 못한 경우</li>
        <li>회사가 서비스를 제공하지 않은 국가에서 비정상적이거나 우회적인 방법을 통해 서비스를 이용하는 경우</li>
        <li>관련 법령에서 금지하는 행위를 할 목적으로 신청 하는 경우</li>
      ...
      <h3>제6조 (약관 외 준칙)</h3>
      <p>이 약관에서 정하지 아니한 사항과 이 약관의 해석에 관하여는 「전자상거래 등에서의 소비자보호에 관한 법률」「약관의 규제에 관한 법률」「정보통신망이용촉진 및 정보보호 등에 관한 법률」「콘텐츠산업진흥법」 등 관련 법령 또는 상 관례에 따릅니다.</p>

      <h3>제7조 (운영정책)</h3>
      <p>① 약관을 적용하기 위하여 필요한 사항과 약관에서 구체적 범위를 정하여 위임한 사항을 서비스 운영정책(이하 “운영정책”이라 합니다)으로 정할 수 있습니다.</p>
      <p>② 회사는 운영정책의 내용을 회원이 알 수 있도록 서비스 내 또는 그 연결화면에 게시합니다.</p>
      <p>③ 운영정책을 개정하는 경우에는 제4조 제2항의 절차에 따릅니다. 다만 운영정책 개정내용이 다음 각 호의 어느 하나에 해당하는 경우에는 제2항의 방법으로 사전에 공지합니다.</p>
      <ul>
      ...
      <p>⑤ 회사는 회원들의 개인정보를 제3자에게 마케팅 및 홍보 제휴 등의 목적으로 제공할 수 있습니다. 수집된 개인정보는 대상이 될 수 있습니다.</p>
      ...
      <ul>
        ...
        <li>회사가 제공하지 않는 서비스나 비정상적인 방법을 통해 특정한 정보 등을 매매 또는 증여하거나 이를 취득하여 이용하는 행위</li>
        ...
        <li>회나 타인의 지적재산권 또는 초상권을 침해하는 행위 타인의 명예를 훼손하거나 손해를 가하는 행위</li>
      ...
      <p>② 회원의 계정 및 모바일 기기에 관한 관리 책임은 회원에게 있으며 이를 타인이 이용하도록 하게 하여서는 안 됩니다. 모바일 기기의 관리 부실이나 타인에게 이용을 승낙함으로 인해 발생하는 손해에 대해서 회사는 책임을 지지 않습니다.</p>
      ...
      <p>② 회사는 회원에게 서비스를 제공할 때 이 약관에 정하고 있는 서비스를 포함하여 기타 부가적인 서비스를 함께 제공할 수 있습니다.</p>
      <p>③ 회사는 회원의 등급을 구분하고 이용시간 이용횟수 제공 서비스의 범위 등을 세분화하여 이용에 차등을 둘 수 있습니다.</p>

      <h3>제12조 (서비스의 이용)</h3>
      <p>① 서비스는 회사의 영업방침에 따라 정해진 시간 동안 제공합니다. 회사는 서비스 제공시간을 애플리케이션 초기화면이나 서비스 공지사항에 적절한 방법으로 안내합니다.</p>
      <p>② 제1항에도 불구하고 회사는 다음 각 호의 경우에는 서비스의 전부 또는 일부를 일시 정지할 수 있습니다. 이 경우 회사는 사전에 그 정지의 사유와 기간을 애플리케이션 초기화면이나 서비스 공지사항 등에 공지합니다. 다만 사전에 공지할 수 없는 부득이한 사정이 있는 경우 사후에 공지할 수 있습니다.</p>
      <ul>
      ...
      <p>① 회사는 원활한 서비스 제공을 위해 운영상 또는 기술상의 필요에 따라 서비스를 변경할 수 있으며 변경 전에 해당 내용을 서비스 내에 공지합니다. 다만 버그․오류 등의 수정이나 긴급 업데이트 등 부득이하게 변경할 필요가 있는 경우 또는 중대한 변경에 해당하지 않는 경우에는 사후에 공지할 수 있습니다.</p>
      <p>② 회사는 영업양도․분할․합병 등에 따른 영업의 폐지 서비스 제공의 계약만료 당해 서비스의 현저한 수익 악화 등 경영상의 중대한 사유로 인해 서비스를 지속하기 어려운 경우에는 서비스 전부를 중단할 수 있습니다.</p>
      <p>③ 제2항의 경우 회사는 사용하지 않았거나 사용기간이 남아 있는 이용권에 대해 제25조 제3항에 따라 환급합니다.</p>
      <p>④ 제2항의 경우 회사는 서비스 중단 이후 30일 이상의 기간을 정하여 그 기간 동안 제3항의 환급절차를 이행하기 위한 전담 창구 등 고객 대응 수단을 마련하여 운영합니다.</p>

      <h3>제14조 (정보의 제공)</h3>
      <p>회사는 유저가 작성한 기록을 다른 유저에게 보여주고 허위 사실 등에 대한 기록은 삭제하거나 보여주지 않을 수 있으며 이미 삭제되거나 가려진 정보는 제공되지 않습니다.</p>

      <h3>제15조 (정보의 수집 등)</h3>
      <p>① 회사는 회원간에 생성된 데이터를 저장․보관할 수 있으며 이 정보는 회사만이 보유합니다. 회사는 회원간의 분쟁 조정 민원 처리 또는 서비스 질서의 유지를 위한 경우에 한하여 제3자는 법령에 따라 권한이 부여된 경우에 한하여 이 정보를 열람할 수 있습니다.</p>
      <p>② 회사 또는 제3자가 제1항에 따라 채팅 정보를 열람할 경우 회사는 사전에 열람의 사유 및 범위를 해당 회원에게 고지합니다. 다만 제10조 제1항에 따른 금지행위의 조사․처리․확인 또는 그 행위로 인한 피해 구제와 관련하여 이 정보를 열람해야 할 경우에는 사후에 고지할 수 있습니다.</p>
      <p>③ 회사는 서비스의 원활하고 안정적인 운영 및 서비스 품질의 개선을 위하여 회원의 개인정보를 제외한 회원의 모바일 기기 정보(설정 사양 운영체제 버전 등)를 수집 ‧ 활용할 수 있습니다.</p>
      ...
      <p>① 회사가 제작한 서비스 내의 콘텐츠에 대한 저작권과 기타 지적재산권 그리고 정보들은 또한 회원들이 작성한 정보의 소유주체도 회사에 귀속합니다.</p>
      <p>② 회원은 회사가 제공하는 서비스를 이용하여 얻은 정보 중에서 회사 또는 제공업체에 지적재산권이 귀속된 정보를 회사 또는 제공업체의 사전 동의 없이 복제⋅전송 등의 방법(편집 공표 공연 배포 방송 2차적 저작물 작성 등을 포함합니다. 이하 같습니다)에 의하여 영리목적으로 이용하거나 타인에게 이용하게 하여서는 안 됩니다.</p>
      <p>③ 회원은 서비스 내에서 보여지거나 서비스와 관련하여 회원 또는 다른 이용자가 애플리케이션 또는 서비스를 통해 업로드 또는 전송하는 텍스트를 포함한 커뮤니케이션 이미지 사운드 및 모든 자료 및 정보(이하 “이용자 콘텐츠”라 합니다.)에 대하여 회사가 다음과 같은 방법과 조건으로 이용하는 것을 허락합니다.</p>
      <ul>
      ...
      <p>④ 서비스 내에서 보여지지 않고 서비스와 일체화되지 않은 회원의 이용자 콘텐츠(예컨대 일반게시판 등에서의 게시물)에 대하여 회사는 회원의 명시적인 동의가 없이 이용하지 않으며 회원은 언제든지 이러한 이용자 콘텐츠를 삭제할 수 있습니다.</p>
      <p>⑤ 회사는 회원이 게시하거나 등록하는 서비스 내의 게시물에 대해 제10조 제1항에 따른 금지행위에 해당된다고 판단되는 경우에는 사전 통지 없이 이를 삭제 또는 이동하거나 그 등록을 거절할 수 있습니다.</p>
      ...
      <p>⑦ 이 조는 회사가 서비스를 운영하는 동안 유효하며 회원 탈퇴 후에도 지속적으로 적용됩니다.</p>

      <h3>제18조 (유료 콘텐츠의 구매 사용기간 및 이용)</h3>
      <p>① 서비스 내에서 회원이 구매한 유료 콘텐츠는 해당 애플리케이션을 다운로드 받거나 설치한 모바일 기기에서만 이용할 수 있습니다.</p>
      <p>② 회원이 구매한 유료 콘텐츠의 이용기간은 구매 시 명시된 기간에 따릅니다. 다만 제13조 제2항에 따라 서비스 중단이 이루어지는 경우 기간의 정함이 없는 유료 콘텐츠의 이용기간은 서비스 중단 공지 시 공지된 서비스의 중단일자까지로 합니다.</p>

      <h3>제19조 (회원에 대한 서비스 이용제한)</h3>
      <p>① 회원은 제10조에 따른 회원의 의무를 위반하는 행위를 하여서는 안 되며 해당 행위를 하는 경우에 회사는 다음 각 호의 구분에 따른 회원의 서비스 이용제한 관련 정보(글 사진 영상 등) 삭제 및 기타의 조치를 포함한 이용제한 조치를 할 수 있습니다. 이용제한 조치가 이루어지는 구체적인 사유 및 절차는 제20조 제1항에 따라 개별 서비스의 운영정책에서 정합니다.</p>
      <ul>
        <li>일부 권한 제한 : 일정기간 채팅 등 일정 권한을 제한</li>
        <li>정보 열람 및 작성 제한 : 일정기간 또는 영구히 회원 이용을 제한</li>
      ...
      <ul>
        <li>허위 사실로 악의적인 정보가 작성되었을 경우</li>
        <li>불법프로그램 사용자 또는 시세 조작 등 위법행위자로 의심되는 경우</li>
        <li>그 밖에 각 호에 준하는 사유로 서비스 이용의 잠정조치가 필요한 경우</li>
      </ul>
      <p>위의 모든 경우에는 작성자의 기록 정보 역시 30일간 열람이 중단됩니다.</p>

      <h3>제20조 (이용제한 조치의 사유와 절차)</h3>
      <p>① 회사는 제19조 제1항에 따른 이용제한 조치의 구체적인 사유 및 절차를 제10조 제1항에 따른 금지행위의 내용․정도․횟수․결과 등을 고려하여 운영정책으로 정합니다.</p>
      ...
      <p>① 회원이 회사의 이용제한 조치에 불복하고자 할 때에는 이 조치의 통지를 받은 날부터 14일 이내에 불복 이유를 기재한 이의 신청서를 서면 전자우편 또는 이에 준하는 방법으로 회사에 제출하여야 합니다.</p>
      <p>② 회사는 제1항의 이의신청서를 접수한 날부터 15일 이내에 불복 이유에 대하여 서면 전자우편 또는 이에 준하는 방법으로 답변합니다. 다만 회사는 이 기간 내에 답변이 어려운 경우에는 그 사유와 처리일정을 통지합니다.</p>
      ...
      <h3>제23조 (청약 철회 등)</h3>
      <p>① 회사와 유료 콘텐츠의 구매에 관한 계약을 체결한 회원은 구매계약일과 콘텐츠 이용 가능일 중 늦은 날부터 7일 이내에 별도의 수수료‧위약금 등의 부담 없이 청약철회를 할 수 있습니다.</p>
      ...
      <ul>
        ...
        <li>결제 시 효용이 결정되는 콘텐츠의 결제행위가 있는 경우</li>
      </ul>
      ...
      <p>⑦ 미성년자가 모바일 기기에서 콘텐츠 구매계약을 체결하는 경우 회사는 법정대리인의 동의가 없으면 미성년자 본인 또는 법정대리인이 그 계약을 취소할 수 있다는 내용을 고지하며 미성년자가 법정대리인의 동의 없이 구매계약을 체결한 때에는 미성년자 본인 또는 법정대리인은 회사에 그 계약을 취소할 수 있습니다. 다만 미성년자가 법정대리인이 범위를 정하여 처분을 허락한 재산으로 콘텐츠를 구매한 경우 또는 미성년자가 속임수로써 자기를 성년자로 믿게 하거나 법정대리인의 동의가 있는 것으로 믿게 한 경우에는 취소할 수 없습니다.</p>
      <p>⑧ 콘텐츠 구매계약의 당사자가 미성년자인지 여부는 결제가 진행된 모바일 기기 결제 실행자 정보 결제 수단 명의자 등을 근거로 판단합니다. 또한 회사는 정당한 취소인지를 확인하기 위해 미성년자 및 법정대리인임을 증명할 수 있는 서류의 제출을 요청할 수 있습니다.</p>
      ...
      <p>③ 애플리케이션의 다운로드 또는 네트워크 서비스의 이용으로 인해 발생한 통신요금(통화료 데이터 통화료 등)은 환급 대상에서 제외될 수 있습니다.</p>
      <p>④ 환급은 서비스를 이용하고 있는 모바일 기기의 운영체제 종류에 따라 각 오픈마켓 사업자 또는 회사의 환급정책에 따라 진행됩니다.</p>
      <p>⑤ 회사는 과오납금의 환급을 처리하기 위해 회원에게서 제공받은 정보를 통해 회원에게 연락할 수 있으며 필요한 정보의 제공을 요청할 수 있습니다. 회사는 회원으로부터 환급에 필요한 정보를 받은 날부터 3영업일 이내에 환급합니다.</p>

      <h3>제25조 (계약 해지 등)</h3>
      <p>① 회원은 언제든지 서비스 이용을 원하지 않는 경우 회원 탈퇴를 통해 이용계약을 해지할 수 있습니다. 회원탈퇴로 인해 회원이 서비스 내에서 보유한 정보에 대한 권리는 모두 사라지게 됩니다. 다만 작성된 정보는 회원이 탈퇴해도 여전히 존재합니다.</p>
      <p>② 회사는 회원이 이 약관 및 그에 따른 운영정책 서비스 정책에서 금지하는 행위를 하는 등 본 계약을 유지할 수 없는 중대한 사유가 있는 경우에는 상당한 기간 전에 최고하고 기간을 정하여 서비스 이용을 중지하거나 이용계약을 해지할 수 있습니다.</p>
      <p>③ 회사는 최근의 서비스 이용일부터 연속하여 1년 동안 회사의 서비스를 이용하지 않은 회원(이하 “휴면계정”이라 합니다)의 개인정보를 보호하기 위해 이용계약을 해지하고 회원의 개인정보 파기 등의 조치를 취할 수 있습니다. 이 경우 조치일 30일 전까지 계약해지 개인정보 파기 등의 조치가 취해진다는 사실 및 파기될 개인정보 등을 회원에게 통지합니다.</p>
      ...
      <p>⑧ 회사는 회원의 허위 정보 작성 등 악의적 이용에 대하여 책임을 지지 않습니다.</p>
      ...
      <p>⑫ 회사는 임시회원이 서비스 이용으로 발생한 손해에 대해서는 책임을 지지 않습니다. 다만 회사의 고의 또는 과실에 의한 경우에는 그러하지 아니합니다.</p>

      <h3>제28조 (회원에 대한 통지)</h3>
      <p>① 회사가 회원에게 통지를 하는 경우 회원의 전자우편주소 전자메모 서비스 내 쪽지 문자메시지(LMS/SMS) 등으로 할 수 있습니다.</p>
      ...
      <p>① 회사는 회원의 편의를 고려하여 회원의 의견이나 불만을 제시하는 방법을 서비스 내 또는 그 연결화면에 안내합니다. 회사는 이러한 회원의 의견이나 불만을 처리하기 위한 전담인력을 운영합니다.</p>
      <p>② 회사는 회원으로부터 제기되는 의견이나 불만이 정당하다고 객관적으로 인정될 경우에는 합리적인 기간 내에 이를 신속하게 처리합니다. 다만 처리에 장기간이 소요되는 경우에는 회원에게 장기간이 소요되는 사유와 처리일정을 서비스 내 공지하거나 제28조 제1항에 따라 통지합니다.</p>
      <p>③ 회사와 회원 간에 분쟁이 발생하여 제3의 분쟁조정기관이 조정할 경우 회사는 이용제한 등 회원에게 조치한 사항을 성실히 증명하고 조정기관의 조정에 따를 수 있습니다.</p>
    </section>
  );
}